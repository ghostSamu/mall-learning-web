    ...
    export default {
        ...
        methods: {
            getAllResourceList(){
                fetchAllResourceList().then(response => {
                   ...
                   for (let i=0; i<this.allResource.length; i++){
                       this.allResource[i].checked = false;  //没看懂 整个check属性是哪儿来的
                   }
            ...
            getAllResourceCateList(){
                listAllCate().then(response => {
                    ...
                    for (let i=0; i<this.allResourceCate.length; i++){
                        this.allResourceCate.checked = false;
                    }
            ...
            isIndeterminate(categoryId){
                let cateResource = this.getResourceByCate(categoryId);
                ...
                return !(checkedCount === 0 || checkedCount === cateResource.length);
            },
            isAllChecked(categoryId){
                let cateResources = this.getResourceByCate(categoryId);
                if (cateResources==null) return null;
                let checkedCount = 0;
